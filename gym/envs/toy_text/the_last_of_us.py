...
class TheLastOfUsEnv(Env):
    ...
    def _render_gui(self, mode):
        ...
        if self.zombie_kill is None:
            file_name = path.join(path.dirname(__file__), "img/zombie_kill.png")
            self.zombie_kill = pygame.transform.scale(
                pygame.image.load(file_name), self.cell_size
            )
        ...
        if desc[bot_row][bot_col] == b"Z":
            self.window_surface.blit(self.zombie_kill, cell_rect)
        else:
